	...
	compassID = navigator.compass.watchHeading(
		function (heading) {
			if (
				...
				heading.magneticHeading.toFixed(0)
			) {
			...
			elemHeading.innerHTML =
				Math.abs(currentDirection.toFixed(0)) + "&#176;";
			// showHotels();
		},
		function (error) {
...
function startup() {
	...
	routeId = null;
	let last_long = parseFloat(
		localStorage.getItem("setting_lastlocationLONG"),
	);
	...
	searchBox = new google.maps.places.SearchBox(
		document.getElementById("SC_SearchBox"),
	);
				...
				function (tx, txresults) {
					...
					if (txresults.rows.length > 0) {
						...
						rtContainer.innerHTML +=
							...
							row.placeid +
							divC +
							limitString(row.hotelname, 25) +
							divD +
							lengthConv(getdistance, "short") +
							divE +
							setBearing(
								bearingCalc(
									currentLongitude,
						...
						let hoteldiv = document.getElementById(
							"SC_" + row.placeid,
						);

						hoteldiv.children[2].children[0].style.color =
							setRatingColor(row.rating);
						...
						directionsService.route(
							{
								...
								travelMode:
									localStorage.getItem("setting_travelmode"),
							},
...
function initSettings() {
	if (typeof Storage !== "undefined") {
		...
		switch (localStorage.getItem("setting_searchRadius")) {
			case "500":
				if (
					localStorage.getItem("setting_unitConversion") == "Imperial"
				) {
				...
				if (txresults.rows.length > 0) {
					...
					for (let i = 0; i < txresults.rows.length; i++) {
						...
						if (
							getdistance <=
							parseInt(
								localStorage.getItem("setting_searchRadius"),
							)
						) {
		...
		tx.executeSql(
			...
			function (tx, txresults) {
				...
				if (txresults.rows.length > 0) {
					...
					let hc_trackbtntext =
						document.getElementById("HC_trackbtntxt");
					let hc_website = document.getElementById("HC_website");
					let hc_googlemaps =
						document.getElementById("HC_googlemaps");
					let hc_address = document.getElementById("HC_address");
					let hc_phonenumber =
						document.getElementById("HC_phonenumber");
