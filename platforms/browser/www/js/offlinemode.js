...
function startCompass(){
	...
	compassID = navigator.compass.watchHeading(
		function(heading){
			...
			element.style.transform = "scale(1.3) rotate(" + Math.abs(val.toFixed(0)) + "deg)";
...
function startgeolocation(){
	...
	if(geoId != null){
		...
		geoId = navigator.geolocation.watchPosition(geoSuccess, geoFailed, gps_options);
	}
	else{
		setBGColor("GPSsymbol", "orange");
...
function startup(){
	...
	switchFocus("REALTIME");
	setVisibility("MainContent", "visible");
	...
	// routeId = null;
	currentLongitude = parseFloat(localStorage.getItem("setting_lastlocationLONG"));
	currentLatitude = parseFloat(localStorage.getItem("setting_lastlocationLAT"));

	// currentLocation = new google.maps.LatLng(last_lat, last_long);
	...
	// searchBox = new google.maps.places.SearchBox(document.getElementById("SC_SearchBox"));

	// map.addListener('bounds_changed', function(){
	// 	searchBox.setBounds(map.getBounds());
	// });

	// searchBox.addListener('places_changed', searchBoxFire);

	let searchinput = document.getElementById("SC_SearchBox");
...
function searchOffline(){
	...
	webdb.transaction(function(tx){
		let input = document.getElementById("SC_SearchBox").value;

		let sqlstr = "SELECT * FROM hotelcollection WHERE hotelname LIKE '%" + input + "%' ORDER BY hotelname";


		tx.executeSql(sqlstr, [], 
			function(tx,txresults){
				let rtContainer = document.getElementById("SC_ResultsArea");

				rtContainer.innerHTML = "";

				let divA = "<div class='RTHotelContainer' id='";
				let divB = "' onclick=checkHotel('"
				let divC = "') ><div class='RTHotelTitle'><div class='RTHotelTitleText'>";
				let divD = "</div></div><div class='RTHotelDistance'>";
				let divE = "<span class='RTHotelCompass'>";
				let divF = "</span></div><div class='RTRating'><div class='RTRatingText'>";
				let divG = "</div></div></div>";
				...
				if(txresults.rows.length > 0){
					...
					for(let i = 0; i < txresults.rows.length; i++){
						...
						rtContainer.innerHTML += 
							divA + ("SC_" + row.placeid) +
							divB + row.placeid +
							divC + limitString(row.hotelname, 25) +
							divD + lengthConv(getdistance, "short") +
							divE + setBearing(bearingCalc(currentLongitude, currentLatitude, row.longitude, row.latitude), "short") +
							divF + setRating(row.rating) +
							divG;

						let hoteldiv = document.getElementById("SC_" + row.placeid);

						hoteldiv.children[2].children[0].style.color = setRatingColor(row.rating);

					}
				}
				else{
					rtContainer.innerHTML = "No results";
				}
...
// 	if(places.length == 0){
// 		rtContainer.innerHTML = "Can't find what you're looking for...";
// 		return;
// 	}
// 	else{
// 		let amount = 0;
...
// 					let thetypes = place.types;

// 					if(txresults.rows.length <= 0){
// 						// console.log(thetypes.includes("lodging"));

// 						if(thetypes.includes("lodging")){

// 							let test = service.getDetails(
...
// 						// 	hoteldiv.style.border = "0.5vh solid orange";
// 						// }
...
// 					}
// 					else{
// 						let sqlstr = "UPDATE hotelcollection SET hotelname=?, latitude=?, longitude=?, rating=?, totalusers=?, address=?, phonenumber=?, intphonenumber=?, globalcode=?, url=?, website=?, last_update=? WHERE placeid=?;";
...
function showHotels(){
	...
	let rtContainer = document.getElementById("RealtimeContent");
			...
			function(tx,txresults){
				...
				if(txresults.rows.length > 0){
					...
					for(let i = 0; i < txresults.rows.length; i++){
						...
						if(getdistance <= parseInt(localStorage.getItem("setting_searchRadius"))){
					...
					if(hotelblock.includes(trackID)){
						hotelblock.splice(hotelblock.indexOf(trackID),1);
					}
					hotelblock.splice(hotelblock.length,0,trackID);
						...
						for(let i = hotelblock.length - 1; i >= 0; i--){
							...
							for(let x = 0; x < txresults.rows.length; x++){
								...
								if(hotelblock[i] == row.placeid){
									...
									let hoteldiv = document.getElementById(row.placeid);
...
//     	includeScript("https://maps.googleapis.com/maps/api/js?key=AIzaSyDBRAJggWx0nDXOAPqJHky_8LIGg6v8cFc&libraries=geometry,places", "Google Maps");

// 	}
// 	else{
//     	setVisibility("RSErrorContainer", "hidden");
//     	setVisibility("ConnectionContainer", "hidden");
//     	setVisibility("MainForm", "visible");
	...
	webdb.transaction(function(tx){
		...
		tx.executeSql(sqlstr, [checkhotelID], 
			function(tx,txresults){
				...
				if(txresults.rows.length > 0){
					...
					hc_distance.innerHTML = lengthConv(distanceCalc(currentLongitude,currentLatitude,row.longitude,row.latitude),"long");
					hc_direction.innerHTML = setBearing(bearingCalc(currentLongitude,currentLatitude,row.longitude,row.latitude),"long");
					...
					if(row.intphonenumber != "undefined"){
						...
						hc_phonenumber.onclick = function(){
							window.plugins.CallNumber.callNumber(callSuccess, callError, row.intphonenumber, false);
						};
					}
					else{
		...
		tx.executeSql(sqlstr, [checkhotelID],
			function(tx,txresults){
				...
				let revA = "<div class='HCReviewContainer'><div class='HCReviewRating'>"
				...
				if(txresults.rows.length > 0){
					...
					if(amount == 0){
						hc_reviewsarea.innerHTML = "<div class='noreviews'>There are no reviews with comments to be shown.</div>"
					}
				}
				else{
					hc_reviewsarea.innerHTML = "<div class='noreviews'>There are no reviews for this place yet.</div>"
				}
...
// 	let node = document.createElement("script"), okHandler, errHandler;
...
// 	    	}
//     		setVisibility("ConnectionContainer", "hidden");
// 			setVisibility("RSErrorContainer", "visible");
...
function switchFocus(input){
	...
	switch(input){
		case "REALTIME":
			// setVisibility("GoogleMaps", "hidden");
			// setBGColor("Focus_MAP", "gainsboro");
			// setTextColor("Text_MAP", "darkslateblue");


			setVisibility("SearchContent", "hidden");
			setBGColor("Focus_SEARCH", "gainsboro");
			setTextColor("Text_SEARCH", "darkslateblue");


			setVisibility("RealtimeContent", "visible");
			setBGColor("Focus_REALTIME", "darkslateblue");
			setTextColor("Text_REALTIME", "white");
		...
		case "SEARCH":
			...
			setVisibility("RealtimeContent", "hidden");
			setBGColor("Focus_REALTIME", "gainsboro");
			setTextColor("Text_REALTIME", "darkslateblue");

			setVisibility("SearchContent", "visible");
			setBGColor("Focus_SEARCH", "darkslateblue");
			setTextColor("Text_SEARCH", "white");
			break;
		// case "MAP":
		// 	setVisibility("SearchContent", "hidden");
		// 	setBGColor("Focus_SEARCH", "gainsboro");
		...
		// 	setVisibility("GoogleMaps", "visible");
		// 	setBGColor("Focus_MAP", "darkslateblue");
		// 	setTextColor("Text_MAP", "white");
		// 	break;
		default:
			console.error("SWITCH FOCUS > ERROR!");
			switchFocus("REALTIME");
			break;
